...
When creating a variable, we would use a principle to follow that helps create a variable name which is accepted by Python. This is known as _naming conventions_[^f2].
...
Before we go on further, I must explain the Python Naming Convention or _rules_. Some names are allowed while others will cause errors.
...
With that said, though what you can store in a variable isn't necessarily restrictive. I still need to know _what_ we can store in there, which leads to the topic of Data Types.

### What are Data Types?

In Computer Science, there are two umbrella terms for Data Types that we need to familiarise ourselves with[^f3].
...
- **Complex Numbers**
  - Complex numbers are used for more advanced mathematical operations, such as representing values on a graph. _I'll be learning about this later and it will have it's own post._
- **Lists**
